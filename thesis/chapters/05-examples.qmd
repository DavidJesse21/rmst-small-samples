...
In their publication the authors analyzed the data separately for the two different glioma types using the log-rank test as their primary analysis tool.
Here, for simplicity, we consider the whole data set and conduct RMST-based analyses.
Again, the marginal Kaplan-Meier estimates of the survival curves for the two treatment regimens are depicted in @fig-glioma.
Moreover, we again compare the results obtained using the different methods based on undadjusted analyses on the one hand and include adjusted pseudo-observations regression models using the age and histology (glioma type) as control variables on the other hand.
The corresponding results are shown in @fig-res-glioma.
...
First and foremost, we can conclude that in this example, for all of the selected cutoff time points ($t^* = 20,\, 30,\, 40$ months), the test decision is the same for all methods, determining a positive treatment effect of the adjuvant therapy.
Among the unadjusted methods the observed differences regarding the width of the confidence intervals are the same as for the previous examples.
For the adjusted methods we can see that the inclusion of the covariates leads to smaller but also more precise (narrower confidence intervals) effect estimates.
