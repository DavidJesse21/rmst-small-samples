...
Here, this is one goal we want to achieve, i.e. highlighting similarities and differences between the methods when applied to the same real-world problem.
...
With respect to the other methods the studentized permutation test suggests the least evidence for concluding that in fact there is a treatment effect followed by the bootstrap approach (see @tbl-res-hellmann).
...
As it can be expected visually form @fig-glioma the log-rank test does not deviate from this decision.
