...
Just like indicated in the previous section, we are in the setting of a randomized controlled trial with a time-to-event endpoint and want to compare an experimental treatment ($j = 1$) to a reference treatment or placebo ($j = 0$).
...
$$ {#eq-test-problem}
In the following subsections we present the different methods presented in this paper for conducting such a test.


## Asymptotic Test {#sec-asy}

In this subsection we present the two-sample test for differences in restricted mean survival times following @hasegawa2020.

For the testing problem in (@eq-test-problem) the following Wald-type test statistic can be constructed:
$$
...
$$ {#eq-test-asy}
Here, $z_{1- \alpha/2}$ denotes the $1 - \alpha/2$-quantile of the standard normal distribution and $\mathbf{1}$ is a binary indicator function.
In line with the statistical test in (@eq-test-asy) a symmetric confidence interval can be constructed:
$$
CI^{\text{asy}} = \left[ \muhat_1(t^*) - \muhat_0(t^*) \mp z_{1- \alpha/2} \sqrt{\Varhat[\muhat_1(t^*)] + \Varhat[\muhat_0(t^*)]} \right]
$$ {#eq-ci-asy}
Of course, for the construction of such a test using the test statistic in (@eq-tstat) we need a suitable point estimator $\muhat(t^*)$ and an associated estimator of its variance $\Varhat[\muhat(t^*)]$.
The point estimator $\muhat(t^*)$ is given by the integral from $0$ to $t^*$ of any appropriate estimator of the survival function:
$$
...
We now present the studentized permutation method introduced by @ditzhaus2023 as an alternative to the asymptotic method presented before.
In principle, this approach is not exceedingly different compared to the asymptotic one.
In fact, it relies on the same non-parametric estimators of the RMST and its variance.
...
\right|    
$$ {#eq-tstat-perm}
We can then carry out the statistical test
$$
\varphi^{\pi} = \mathbf{1} \{|Z| > q_{1 - \alpha}^{\pi} \}
\, ,
$$ {#eq-test-perm}
where $q_{1 - \alpha}^{\pi}$ denotes $(1 - \alpha)$-quantile of the the studentized  permutation test statistics.
Similarly as for the asymptotic approach, a confidence interval can be constructed by simply replacing $z_{1 - \alpha/2}$ in (@eq-ci-asy) by $q_{1 - \alpha}^{\pi}$:
$$
...
Furthermore, when @horiguchi2020a first introduced a permutation test for two-sample RMST tests they extensively discussed how to handle the scenario in which for a permuted data set either $\hat{S}_0(t^*)$ or $\hat{S}_1(t^*)$ is not uniquely defined because the largest event time in that group is smaller than $t^*$ and censored.
Following @ditzhaus2023 based on the results by @horiguchi2020a we use the "horizontal extension" strategy in which we set $\hat{S}_j(t^*) = \hat{S}_j(t_{\text{max}})$ in these cases.
Here, $t_{\text{max}}$ denotes the largest observed event time within the respective group.


## Pseudo-Observations {#sec-pseudo}

### General Idea {#sec-po-general}
...
The idea and motivation is to move away from hazard-based regression models for time-to-event data when we are actually interested in obtaining effect estimates for other quantities, e.g. the ones introduced in @sec-basics and @sec-rmst.
In this regard, hazard-based regression models may impede estimation, inference and interpretation of such effects.
E.g. if we are interested in the RMST difference between two treatment groups, a hazard-based regression model would need to be converted to the survival function first and then again be integrated to obtain an estimate of the RMST.
...
Moreover, such models might underlie assumptions we would actually like to avoid such as proportional hazards.
...
Moreover, $\hat{\theta}_{-i}$ is the same kind of estimator but leaving out the $i$-th observation, i.e. using the $i$-th *jackknife sample* [@efron1993, chapter 1].
...
It is important to note, though, that the estimation of the model (@eq-pseudoglm) proceeds using the pseudo-observations (@eq-pseudo) for all individuals $i = 1, \ldots, n$, regardless whether $V_i$ could actually be computed or not.
The theoretical justification of using pseudo-observations as a response variable in a GLM is their unbiasedness (given an unbiased marginal estimator $\hat{\theta}$) (@andersen2010):
$$
\begin{split}
...
Using the pseudo-observations (@eq-pseudo) relies on the assumption that the censoring mechanism is independent of any covariates.
...
Another important aspect to keep in mind is that the formulation of a typical generalized linear model $\expect[y] = g^{-1}(\xvec_i' \betavec)$ is usually motivated and determined by (conditional) distributional assumptions about the response variable $y$ [@fahrmeir2013, Chapter 5.4].
...
Second, the motiavtion of the regression model (@eq-pseudoglm) in the first place is in obtaining effect estimates $\betavec$ with a particular interpretation.
...
Due to this special situation we employ quasi-likelihood methods instead [@fahrmeir2013, Chapter 5.5].
All requirement that need to be met are a correct specification of the expectation structure (@eq-pseudoglm) together with a *working variance* structure $\sigma^2_i$.
...
This estimating equation is just similar to the score function of a likelihood-based model [@fahrmeir2013, Chapters 5.4 and 5.5].
...
Similarly, an asymptotic test (@eq-test-asy) and confidence interval (@eq-ci-asy) can be obtained using the quantiles of the standard normal distribution as the reference under the null hypothesis.
...
Although the concept of a sandwich type covariance matrix estimator can be formulated more generally [@zeileis2006], here we keep using the notation by @fahrmeir2013[Chapter 5.5] for the application to quasi-likelihood models:
$$
\Covhat(\hat{\betavec}) = 
...
\, ,
$$ {#eq-weights-HC3}
where $\residhat_i$ is the working residuals and $h_i$ the leverage of the $i$-th observation, respectively.
Using the diagonal elements of (@eq-covhc) for obtaining standard error estimates we can now compute the test statistic (@eq-tstat-po) and conduct inference similar as in (@eq-test-asy) and (@eq-ci-asy).


### Bootstrap Hypothesis Test {#sec-po-boot}

In comparison to the asymptotic method described in @sec-asy the approach using pseudo-observations might have the advantage of obtaining more accurate estimates of standard errors in finite sample settings.
...
Therefore, we now intend to incorporate a similar feature to the approach using pseudo-observations.
In principle, we could just focus on $\beta_1$ from the model (@eq-rmstglm) and apply a studentized permutation approach similar to the one employed by @ditzhaus2023.
However, as we argue that the main advantage of an approach based on pseudo-observations is its possibility to be extended to more general settings, e.g. one incorporating a continuous control variable, we want any adaption to this method to retain that flexibility as well.

The idea of this approach is similar to that of the studentized permutation test: 
Instead of making a fixed assumption about the approximate distribution of the test statistic under the null hypothesis, in this case (@eq-tstat-po) following a standard normal distribution, and using this assumption even in small sample scenarios, we strive to estimate that distribution from the observed data directly.
...
In the simplest setting, as it is also the case here, $\xmat$ could as well be just a vector, e.g. a vector of treatment indicators.
Furthermore, let $(\tvec, \deltavec, \xmat)^b \equiv \{(t_i, \delta_i, \xvec_i')^b: \, i = 1, \ldots, n \})$ denote a similar data set that has been obtained by case resampling with replacement, which we call *bootstrap sample*.
Such a bootstrap sample will be generated $B$ times, just as it would be done for the permutation data sets $(\tvec, \deltavec)^{\pi}$ (@sec-studperm).
For each of the $B$ bootstrap samples we now seek to calculate the pseudo-observations (@eq-pseudo) and estimate a corresponding regression model (@eq-pseudoglm).
Finally, for any of coefficient $\beta$ of such a model we calculate the bootstrap test statistic
$$
Z^b = \left| \frac{\betahat^b - \betahat}{\sehat(\betahat^b)} \right|
\, .
$$ {#eq-tstat-boot}
While $\betahat$ denotes the estimate of $\beta$ that has been obtained using the original data, all components with $b$ in the superscript are calculated based on the $b$-th bootstrap sample using the same methods as the ones presented in @sec-po-asy.
The rationale of this formula is that "the estimate of $\beta$ from the bootstrap samples should, on average, be equal to $\betahat$, at least asymptotically" [@mackinnon2009] and therefore $Z_b$ should take values that lead to a retention of the null hypothesis.
...
Due to the computation of the pseudo-observations even the asymptotic test (TODO) already incorporates a resampling scheme.
Applying a bootstrap on top of that implies a nested resampling procedure, which gets computationally more expensive (a) the larger the original sample is and (b) the more bootstrap samples we want to draw, i.e. the more accurate we want our statistical test to be.
Such a problem becomes apparent quickly.
E.g. if we had a sample of size $n = 50$ and wanted to draw $B = 1000$ bootstrap samples, then we would need to carry out $n \times B = 50000$ computations in total.
One way to tackle this problem could be to optimize the computational implementation of corresponding functions or to embrace parallel computing capabilities.
However, a more subtle approach may be to try to eliminate one of the two resampling levels.
This is indeed possible as the pseudo-observations can also be calculated using an approximation technique:
$$
\begin{split}
  P_i &= n \hat{\theta} - (n - 1) \hat{\theta}_{-i} \\
...
\end{split}
$$ {#eq-pseudo-approx}
This approximation requires that the estimator $\hat{\theta}$ can be written as a function of weights $w_i$ attached to each individual $i = 1, \ldots, n$ in the data.
