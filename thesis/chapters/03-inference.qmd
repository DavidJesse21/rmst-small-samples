...
As indicated in the previous section, we are in the setting of a randomized controlled trial with a time-to-event endpoint and want to compare an experimental treatment ($j = 1$) to a reference or placebo treatment ($j = 0$).
...
$$ {#eq-tstat}
Using the martingale central limit theorem in combination with the functional $\delta$-method it can be shown that, asymptotically, this test statistics follows a standard normal distribution under the null hypothesis [@zhao2016], i.e.
$$
Z \danull \dnorm(0, 1) \, .
$$ {#eq-dapprox}
...
For instance, $f(T_i | t^*) = \min(T_i, t^*)$ and the expectation thereof correspond to the restricted mean survival time.
...
The advantages of these are that they do not exhibit any modelling assumptions and are usually fast to compute.
...
@fahrmeir2013[Chapter 5.5] give a unique definition of $\hat{\mmat}$, but we shall let it unspecified at the moment, allowing for different types of sandwich estimators.
...
Therefore, we now intend to incorporate a similar feature into the pseudo-observations approach using pseudo-observations.
