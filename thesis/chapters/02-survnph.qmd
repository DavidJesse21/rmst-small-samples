...
\end{split}
$$
Here, we can see that the Weibull distribution exhibits proportional hazards if the shape parameter $\alpha$ is the same for both populations.
...
@fig-nph depicts theoretical scenarios only that might be considered idealized to some extent.
Still, we can also use real-world data sets to contrast situations with hazards that are rather proportional or non-proportional over time.
Such examples are given in @fig-nph2 where estimated survival and hazard functions are shown.
...
Moreover, the time-varying hazard ratio derived from these estimates of the hazard functions are portrayed in the bottom row of the plot.
...
Accordingly, the estimated hazard functions are proportional to most extent.
Although the hazard ratio fluctuates somewhat in the initial phase of the study, it remains within a relatively small range overall, not deviating much from the fixed hazard ratio obtained from the Cox model.
...
While the hazard function of the dacarbazine group monotonically increases over time, we can see that for the nivolumab group it increases in the first 2.5 months but decreases afterwards.
Also, the curve of the time-dependent hazard ratio exhibts greater variability.
...
For the nonparametric Kaplan-Meier estimator of the survival function to reach $0$ and therefore being able to estimate an unrestricted mean, the largest event time needs to be uncensored, which is rarely the case.
...
\end{cases}
$$
Here, $t_j$ ($j = 1, 2, \ldots, m$) denote the ordered and distinct event times observed in the survival data.
...
An estimator for the variance of (@eq-pointest) based on Greenwood's formula is given by [@hasegawa2020]
$$
