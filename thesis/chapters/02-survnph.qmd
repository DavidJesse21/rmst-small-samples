...
Correspondingly, $\xmat \in \real^{n \times (p+1)}$ denotes the design matrix for a whole data set.[^1]
...
Among the statistical models that employ the proportional hazards assumption perhaps the best-known and most widely used one is the semiparametric *Cox proportional hazards model* [@cox1972]:
$$
...
\end{cases}
$$
Here, $t_j$ ($j = 1, 2, \ldots, m$) denote the ordered and ditinct event times observed in the survival data.
...
$$ {#eq-pointest}
where as before $t_1 < t_2 < \ldots < t_D$ denote the ordered and distinct event times observed in the survival data and $t_{D+1} = t^*$.
