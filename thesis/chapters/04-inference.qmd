...
This is indeed possible as we can obtain an approximate version of the pseudo-observations:
$$
\begin{split}
  P_i &= n \hat{\theta} - (n - 1) \hat{\theta}_{-i} \\
...
\end{split}
$$ {#eq-pseudo-approx}
This is known as the *infinitesimal jackknife* (IJ) pseudo-observations and requires that the estimator $\hat{\theta}$ can be written as a function of weights $w_i$ attached to each individual $i = 1, \ldots, n$ in the data.
...
Despite this, the loss in numerical accuracy is negligible even for moderate sample sizes, making the IJ pseudo-observations an attractive alternative to ordinary pseudo-observations.
