...
box::use(
  ...
  survival[Surv, survfit, coxph],
  ggsurvfit[survfit2, ggsurvfit],
  muhaz[muhaz],
  fs,
...
p_s1 = plot_fpm(m1, t_eval1, "survival", linewidth = 1.1)
...
# Survival functions based on FPM
p_s2 = plot_fpm(m2, t_eval2, "survival", linewidth = 1.1)
p_h2 = plot_fpm(m2, t_eval2, "hazard", linewidth = 1.1)
p_hr2 = plot_fpm(m2, t_eval2, "HR", linewidth = 1.1, hr_order = 0:1)
...
wrap_plots(
  # Proportional hazards
  p_km1, p_h1, p_hr1 + xlab(NULL),
  # Non-proportional hazards
  p_km2, p_h2, p_hr2 + xlab(NULL)
) +
