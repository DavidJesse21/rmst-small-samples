...
if (!fs$file_exists(fs$path("thesis", "objects", "res_hellmann", ext = "rds"))) {
  
  # Methods ----
  
  # In line with Ditzhaus (2023)
  cutoffs = c(12, 15, 18)
  ...
  # All results
  dt_res = rbindlist(list(dt_asy, dt_studperm, dt_po_asy, dt_po_boot))
  
  saveRDS(dt_res, fs$path("thesis", "objects", "res_hellmann", ext = "rds"))
