...
if (!fs$file_exists(fs$path("thesis", "objects", "res_glioma", ext = "rds"))) {
  ...
  res_po_asy2 = lapply(cutoffs, function(cutoff) {
    m = rmeanglm(
      Surv(time, event) ~ group + age + histology, time = cutoff, data = dt,
      model.censoring = pseudo_strat, formula.censoring = ~ group
    )
  ...
  saveRDS(dt_res, fs$path("thesis", "objects", "res_glioma", ext = "rds"))
} else {
  # Just read results
